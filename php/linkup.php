<?php
/* 
This is the core hw file where most of the
...
add_action('after_setup_theme','hw_start', 16);
...
function hw_start() {
    ...
    add_filter( 'wp_head', 'hw_remove_wp_widget_recent_comments_style', 1 );
    ...
    // adding the hw search form (created in functions.php)
    add_filter( 'get_search_form', 'hw_wpsearch' );
...
function hw_head_cleanup() {
  ...
  add_filter( 'style_loader_src', 'hw_remove_wp_ver_css_js', 9999 );
...
function hw_remove_wp_widget_recent_comments_style() {
...
function hw_remove_recent_comments_style() {
...
function hw_related_posts() {
	...
	if($tags) {
        ...
        if($related_posts) {
            ...
            <?php echo '<li class="no_related_post">' . __( 'No Related Posts Yet!', 'hwtheme' ) . '</li>'; ?>
		<?php }
	}
	wp_reset_query();
	echo '</ul>';
...
function hw_page_navi($before = '', $after = '') {
	...
	if ($start_page >= 2 && $pages_to_show < $max_page) {
		$first_page_text = __( "First", 'hwtheme' );
	...
	echo '</li>';
	if ($end_page < $max_page) {
		$last_page_text = __( "Last", 'hwtheme' );
